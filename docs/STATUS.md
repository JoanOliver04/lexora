...
**Estado de la tarea:** LEX-5.1…5.5 `HECHO` · siguiente LEX-5.6 · Q-005 abierta (opción 1 aplicada, reversible, visible en la UI de mazos desde LEX-3.5) · Q-006 abierta (sin cascada, no bloqueante, documentada y probada en LEX-3.8; condiciona LEX-5.6)
**Rama / commit base / HEAD:** `main` en `81f5d0b` (PR #77, LEX-5.5). Sin rama de trabajo activa.
...
Informe en [`evidence/LEX-5.5.md`](evidence/LEX-5.5.md). PR #77 fusionada a
`main` (merge `81f5d0b`); CI verde en los tres trabajos, runs `34602539398`
(PR) y `34602997962` (merge). Migración
`20260911150000_study_rls`.

Políticas de dueño en las tres tablas. `review_logs` sin `UPDATE`.
Índices de cola `(owner_id, due_at)` e historial. pgTAP `130` (45):
A no alcanza lo de B ni por UUID; el dueño no edita su log.

```text
pnpm db:reset  11 migraciones + seed desde vacío
pnpm db:test   14 ficheros / 439 aserciones, PASS (130: 45)
pnpm check     exit 0 (format, lint, typecheck, contraste 18/18, vitest 49/312 + 1 skipped, build)
```

Fuera de alcance declarado: alta de `LearningState` (LEX-5.6); UI.

### LEX-5.4 — Migraciones de estudio — `HECHO`

Informe en [`evidence/LEX-5.4.md`](evidence/LEX-5.4.md). PR #75 fusionada a
`main` (merge `10a391f`); CI verde en los tres trabajos, runs `34600614752`
(PR) y `34601083487` (merge). Migración
`20260911120000_study_schema`.

Tres tablas: `learning_states`, `study_sessions`, `review_logs`. RLS
habilitado, sin políticas (LEX-5.5). Mapeo campo a campo; `elapsed_days`
omitido; `learning_step` incluido. Un estado por ítem; clave de
idempotencia por dueño. Archivar un ítem no borra memoria.

```text
pnpm db:reset  10 migraciones + seed desde vacío
pnpm db:test   13 ficheros / 395 aserciones, PASS (120: 83)
pnpm check     exit 0 (format, lint, typecheck, contraste 18/18, vitest 49/312 + 1 skipped, build)
```

Fuera de alcance declarado: políticas e índices de cola (LEX-5.5); UI.

### LEX-5.3 — Configuración FSRS v1 — `HECHO`

Informe en [`evidence/LEX-5.3.md`](evidence/LEX-5.3.md). PR #73 fusionada a
`main` (merge `6ff8b5d`); CI verde en los tres trabajos, runs `34596435352`
(PR) y `34596876298` (merge). **Sin migración.**

`V1_SCHEDULER_CONFIG` (`configVersion: v1`): retención 0,90, pasos
`1m`/`10m`, fuzz encendido, 21 pesos FSRS-6 copiados. Zod en el borde.
El adaptador pasa `w` explícitos.

```text
pnpm check    exit 0 (format, lint, typecheck, contraste 18/18, vitest 49/312 + 1 skipped, build)
```

Fuera de alcance declarado: persistencia (LEX-5.4); UI.
...
Informe en [`evidence/LEX-5.1.md`](evidence/LEX-5.1.md). PR #69 fusionada a
`main` (merge `74959e6`); CI verde en los tres trabajos, runs `34589353696`
(PR) y `34589815103` (merge). **Sin migración.**

`ts-fsrs@5.4.2` fijado (FSRS-6.0). Node `>=20`. Sin UI ni adaptador.
El dominio sigue sin importar la librería.

- Spike: estados, ratings (Manual no es Grade), pasos `1m`/`10m`,
  fuzz sembrado, parámetros JSON, `Date` a mapear.
- No se usa el beta 6 ni el optimizador.

```text
pnpm check    exit 0 (format, lint, typecheck, contraste 18/18, vitest 45/294 + 1 skipped, build)
```

Fuera de alcance declarado: puerto/adaptador (LEX-5.2); config de
producto (LEX-5.3).
